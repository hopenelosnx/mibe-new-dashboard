...
import { getAllCulturalEvents, CulturalEvent, addCulturalEvent, updateCulturalEvent, deleteCulturalEvent } from '@/services/dataService';
...
const CulturalEventPage = () => {
  ...
  const [currentCulturalEvent, setCurrentCulturalEvent] = useState<CulturalEvent | null>(null);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [CulturalEventsToDelete, setCulturalEventsToDelete] = useState<number | null>(null);
...
const handlePageChange = (newPage: number) => {
  ...
  const additionalFields = [
    {name:"title", label:"Title", type:"text", placeholder:"Enter Cultural Events title", required:true},
    {name:"published", label:"Published", type:"switch", placeholder:"Select Cultural Events published status", required:false},
    {name:"status", label:"Status", type:"option", placeholder:"Enter Cultural Events status",required:true, options: [
      { value: 'Active', label: 'Active' },
      { value: 'Inactive', label: 'Inactive' },
      { value: 'Archived', label: 'Archived' },
      { value: 'Draft', label: 'Draft' }
    ]},
    {name:"description", label:"Description", type:"textarea", placeholder:"Enter Cultural Events description",required:true},
    {name:"price", label:"Price", type:"number", placeholder:"Enter Cultural Events price",required:true},
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Travel Cultural Events</h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center p-12 border rounded-lg bg-muted/30">
          <h3 className="text-xl font-semibold mb-2">No Cultural Events found</h3>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {CulturalEvents.map((Culture) => (
            <ListingCard
              ...
              image={typeof Culture.image_url === "string" ? Culture.image_url : "https://via.placeholder.com/150"}
              onEdit={handleEdit}
                ...
                <TableBody>
                    {CulturalEvents.map((Culture) => (
                        <TableRow key={Culture.id}>
                            ...
                            <TableCell className="text-right">{getStatusBadge(Culture.published)}</TableCell>
      ...
      {!currentCulturalEvent && (
        <ListingForm
          ...
          initialValues={{title:"",description:"",content:"",image_url:"", price:0, published:"1"}}
